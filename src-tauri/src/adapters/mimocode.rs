...
/// 列出 MiMoCode message 表中已知 session id
...
/// 按 session created cursor 查询 MiMoCode message row
...
/// 按 session updated cursor 查询 MiMoCode message row
...
/// 查询旧 global watermark 前最后一条 MiMoCode message created cursor
...
mod tests {
    ...
    #[test]
    fn created_cursor_query_uses_session_time_created_id_index() {
        let dir = tempfile::tempdir().unwrap();
        let db_path = dir.path().join("mimocode.db");
        let conn = rusqlite::Connection::open(&db_path).unwrap();
        conn.execute_batch(
