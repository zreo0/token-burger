...
/// MiMoCode message 表的最小行为解析输入
#[derive(Debug, Clone)]
pub struct MiMoCodeMessageRow {
    pub id: String,
...
/// 从 MiMoCode message row 中解析第一版完成事件
pub fn parse_message_row(row: &MiMoCodeMessageRow) -> Option<AgentBehaviorEvent> {
    ...
    Some(AgentBehaviorEvent::new(
        ...
        Some(row.id.clone()),
        None,
        "MiMoCode finished the current turn",
    ))
}
...
mod tests {
    ...
    #[test]
    fn parses_mimocode_finish_stop() {
        let row = MiMoCodeMessageRow {
            id: "msg-1".to_string(),
        ...
        assert_eq!(event.agent_name, "mimocode");
        assert_eq!(event.kind, AgentBehaviorKind::RunCompleted);
        assert_eq!(event.session_id, "session-1");
        assert_eq!(event.summary, "MiMoCode finished the current turn");
    }

    #[test]
    fn skips_mimocode_permission_or_non_assistant_rows() {
        let user_row = MiMoCodeMessageRow {
            id: "msg-1".to_string(),
            session_id: None,
            data: r#"{"role":"user","finish":"stop"}"#.to_string(),
            time_created: 1_780_000_000_000,
        };
        let permission_row = MiMoCodeMessageRow {
            id: "msg-2".to_string(),
            session_id: None,
            data: r#"{"role":"assistant","finish":"permission"}"#.to_string(),
            time_created: 1_780_000_000_000,
        };

        assert!(parse_message_row(&user_row).is_none());
        assert!(parse_message_row(&permission_row).is_none());
    }
}
